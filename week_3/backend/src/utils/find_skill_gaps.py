...
def parse_skills(skill_text: str) -> List[str]:
    text = skill_text.lower().replace("a/b testing", "ab_testing_placeholder").replace("ci/cd", "cicd_placeholder")
    raw_skills = re.split(r'[,/]', text)
    cleaned = {s.strip().replace("ab_testing_placeholder", "a/b testing").replace("cicd_placeholder", "ci/cd") 
               for s in raw_skills if s.strip()}
    return list(cleaned)

# --- CORE ENGINE ---
def find_skill_gaps(resumes_dict: Dict[str, str], db_url: str, model_name: str) -> MultiResumeAnalysis:
    batch_size, cooldown = get_model_limits(model_name)
    resume_items = list(resumes_dict.items())
    chunks = [dict(resume_items[i:i + max(1, batch_size)]) for i in range(0, len(resume_items), max(1, batch_size))]

    # 1. Fetch & Prepare DB data
    conn = sqlite3.connect(db_url)
    cursor = conn.cursor()
    cursor.execute("SELECT tech_stack FROM jobs WHERE tech_stack IS NOT NULL")
    db_skills_pool = [skill for row in cursor.fetchall() for skill in parse_skills(row[0])]
    conn.close()
    ...
    for chunk in chunks:
        ...
        for filename, skills_list in raw_data.items():
            ...
            all_results.append(IndividualGapResult(
                filename=filename,
                gaps=sorted_gaps,
                demand_statistics=dict(sorted(gap_stats.items(), key=lambda x: x[1], reverse=True))
            ))
